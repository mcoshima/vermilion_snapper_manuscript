...
## Copy files into boot directory

## Depending on order of scripts, this might already be done ###############################
files. <- list("vs_envM2.dat", "control.ss_new", "forecast.ss", "starter.ss", "ss.exe")
files.path <- file.path(dir., files.)
file.copy(files.path, file.path(dir.boot), overwrite = TRUE)
file.rename(file.path(dir.boot, "control.ss_new"), file.path(dir.boot, "vs.ctl"))
############################################################################################
...
### Catch df
  ...
  catch = rep(0, n.years.fwd*nrow(catch.patterns)),
