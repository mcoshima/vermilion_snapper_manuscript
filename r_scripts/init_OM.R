...
create_OM <- function(OM_out_dir,
  ...
  ## changed this to my run_ss function bc the original one needs ss to be in path so it was calling wrong one
  run_ss(OM_out_dir, os = os, "-maxfn 0 -phase 50 -nohess")
  ...
  ## tried to incorporate this into the add_sample_struct function
  ##dat[["envdat"]] <- rbind(dat[["envdat"]], new_envdat)
...
#' @template seed
#' @param os specify what operating system it is being run on ("win" for windows, "osx" for mac, and "lin" for linux)
...
run_OM <- function(OM_dir,
  ...
  run_ss(OM_dir, os = os, "-maxfn 0 -phase 50 -nohess")
...
add_sample_struct <- function(sample_struct, dat) {
  ...
  if(!is.null(dat[["envdat"]]) & !is.null(sample_struct[["envdat"]])) {
      tmp_envdat <- sample_struct[["envdat"]]
      tmp_envdat <- tmp_envdat[tmp_envdat[["Yr"]] >= subset_yr_start & tmp_envdat[["Yr"]] <= subset_yr_end, ]
      if (nrow(tmp_envdat) > 0) {
      dat[["envdat"]] <- rbind(dat[["envdat"]], tmp_envdat)
    }
  }
