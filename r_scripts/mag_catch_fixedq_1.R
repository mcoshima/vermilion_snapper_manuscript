...
for (year in Year.vec) {
  ...
  if (recommend_catch == F) {
    .datcatch[year,1] <- rtruncnorm(1, a = 0, b = Inf, c_1[1], 304.26)
    .datcatch[year,2] <- rtruncnorm(1, a = 0, b = Inf, c_2[1], 102.4393)
    .datcatch[year,3] <- rtruncnorm(1, a = 0, b = Inf, c_3[1], 122.6127)
  ...
  if (year %% 10 == 0) {
    ...
    rebuild_year <- dat.list$year_seq[year] + T.target
    
    if (rebuild == T & dat.list$year_seq[year] >= rebuild_year) { 
      T.target <-
    ...
    if (rebuild == T & dat.list$year_seq[year] < rebuild_year) { 
      ...
      OFL[[year]] <- rep.file$derived_quants %>% 
        ...
        separate(Label, into = c("Label",  "Year"), sep = "_") %>% 
        mutate(Year = as.numeric(Year)) %>% 
        filter(Year > floor(dat.list$year_seq[year]) & 
                 Year <= floor(dat.list$year_seq[year])+5) %>% 
        select(Value) %>% 
        pull()
      ...
      c_1[x:(x+4)] <- ABC * dat.list$catch_proportions[1]
      c_2[x:(x+4)] <- ABC * dat.list$catch_proportions[2]
      c_3[x:(x+4)] <- ABC * dat.list$catch_proportions[3]
