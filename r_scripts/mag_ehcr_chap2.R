...
for (year in Year.vec) {
  ...
  if (recommend_catch == F) {
    ...
    c_1[x+1] <- tac  * dat.list$catch_proportions[1]
    c_2[x+1] <- tac  * dat.list$catch_proportions[2]
    c_3[x+1] <- tac  * dat.list$catch_proportions[3]
    ...
    if(fc.mt < c_3[x+1]){
      ...
      rec <- sum((c_3[x+1] * mpatage )/ waa.sub)
    }
    
    .datcatch[year,1] <- rtruncnorm(1, a = 0, b = Inf, mean = c_1[x+1], c_1[1])
    .datcatch[year,2] <- rtruncnorm(1, a = 0, b = Inf, mean = c_2[x+1], c_2[1])
    .datcatch[year,3] <- rtruncnorm(1, a = 0, b = Inf, mean = rec, 100)
  ...
  if (recommend_catch == T) {
    ...
    if(c_1[x+1] == 0 & c_2[x+1] == 0 & c_3[x+1] == 0){
      .datcatch[year, 1] <- 0
      .datcatch[year, 2] <- 0
      .datcatch[year, 3] <- 0
      .datcatch[year, 4] <- runif(1, c_4[1], c_4[2])
  ...
  #total catch in biomass for the year (used to calculate next year's catch)
  tot.catch.b[year] <- sum(.datcatch[year, 1], .datcatch[year, 2], fc.mt, .datcatch[year,4]*wtatage[1,2])
  ...
  if (year %% 10 == 0) {
    ...
    if(bratio < 0.3){
      ...
      OFL[[year]] <- tot.catch.b[year] * ((ind.w.mat[escen,1] * 1+ind.slopes$year[1]) +
