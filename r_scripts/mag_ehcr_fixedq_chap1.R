...
report. <- MO_SSoutput(dir = file.path(dir., "initial_pop", "initial_w_compF", "fixed_q"))

dat.init <- SS_readdat(file.path(dir., "initial_pop", "initial_w_compF", "fixed_q", "VS.dat"))
...
fixed.comp.f <- report.$exploitation %>% filter(Yr == start.year) %>% select(COMP) %>% pull() * proj.index$RS_relative
...
start.status <- report.$derived_quants %>% 
...
if(start.status$Value < 0.3){
  recommend_catch = T
  OFLCatch <- report.$derived_quants %>% 
    filter(str_detect(Label, "ForeCatch_")) %>% 
    select(Label, Value) %>% 
    separate(Label, into = c("Lable", "Year"), sep = "_") %>% 
    mutate(Year = as.numeric(Year)) %>% 
    filter(Year >= start.year & Year <= start.year + 4)
  P.star <- p_star()
  ABC <- OFLCatch$Value * P.star
  
  c_1[2:5] <- ABC * dat.list$catch_proportions[1]
  c_2[2:5] <- ABC * dat.list$catch_proportions[2]
  c_3[2:5] <- ABC * dat.list$catch_proportions[3]
}else{
  recommend_catch = F
}
...
file.copy(file.path(dir.,"/initial_pop/initial_w_compF/fixed_q", files.), file.path(dir.it), overwrite = T)
ss3.file <- list("SS3")
file.copy(file.path(dir., "initial_pop/initial_w_compF/fixed_q/", ss3.file), file.path(dir.it), overwrite = T)

for (year in Year.vec) {
  
  if (recommend_catch == F) {
    tac <- init.catch * ((ind.w.mat[escen,1] * 1+init.slopes$year[1]) +
                           (ind.w.mat[escen,2] * 1+init.slopes$year[2]) +
                           (ind.w.mat[escen,3] * 1+init.slopes$year[3]) +
                           (ind.w.mat[escen,4] * 1+init.slopes$year[4]) +
                           (ind.w.mat[escen,5] * 1+init.slopes$year[5]) +
                           (ind.w.mat[escen,6] * 1+init.slopes$year[6]) +
                           (ind.w.mat[escen,7] * 1+init.slopes$year[7]))
    
    c_1[x] <- tac  * dat.list$catch_proportions[1]
    c_2[x] <- tac  * dat.list$catch_proportions[2]
    c_3[x] <- tac  * dat.list$catch_proportions[3]
    
    .datcatch[year,1] <- rtruncnorm(1, a = 0, b = Inf, mean = c_1[x], 304.26)
    .datcatch[year,2] <- rtruncnorm(1, a = 0, b = Inf, mean = c_2[x], 102.4393)
    .datcatch[year,3] <- rtruncnorm(1, a = 0, b = Inf, mean = c_3[x], 122.6127)
  ...
  if (recommend_catch == T) {
    ...
    if(c_1[x] == 0 & c_2[x] == 0 & c_3[x] == 0){
      .datcatch[year, 1] <- 0
      .datcatch[year, 2] <- 0
      .datcatch[year, 3] <- 0
      .datcatch[year, 4] <- runif(1, c_4[1], c_4[2])
    }else{
      .datcatch[year,1] <- rtruncnorm(1, a = 0, b = Inf, mean = c_1[x], 150)
      .datcatch[year,2] <- rtruncnorm(1, a = 0, b = Inf, mean = c_2[x], 150)
      .datcatch[year,3] <- rtruncnorm(1, a = 0, b = Inf, mean = c_3[x], 150)
      .datcatch[year,4] <- runif(1, c_4[1], c_4[2])
    }
  ...
  if (year %% 10 == 0) {
    ...
    if(bratio < 0.3){
      ...
      OFL[[year]] <- past.catch * ((ind.w.mat[escen,1] * 1+ind.slopes$year[1]) +
                                     ...
                                     (ind.w.mat[escen,6] * 1+ind.slopes$year[6]) +
                                     (ind.w.mat[escen,7] * 1+ind.slopes$year[7]))
    }
