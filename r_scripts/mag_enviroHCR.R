...
fecund <- report.$ageselex %>%
  ...
  proj.index <-
    proj.index.full %>% select(Yr, RS_relative) %>% filter(Yr > 2013 & Yr < 2117) %>% rename(Year = Yr)
  proj.index$low <- NA
  proj.index$med <- NA
  proj.index$high <- NA
  ...
  for (year in Year.vec) {
    ...
    if (recommend_catch == F) {
      ...
      .datcatch[year,2] <- rtruncnorm(1, a = 0, b = Inf, mean = c_2[x], 60.45)
