...
start.year <- 2017
###remove extra files function
remove_extra_files <- function(maindir., scenario){
  for(i in 1:100){
    
    dir. <- file.path(maindir., scenario, i)
    files <- list.files(dir.)
    to.remove <- files[which(str_detect(files, paste(c(".csv", ".5"),collapse = '|'), TRUE) == TRUE)]
    unlink(paste(dir., to.remove, sep = "/"))
...
head(recruit)
rhat <-
  geometric.mean(recruit$pred_recr[c(44:68)]) #recruitment estimates from 1993 to 2014, (44:68 for 1993 to 2017)
rsig <-
...
proj.index <-
  proj.index.full %>% select(Yr, RS_relative) %>% filter(Yr >= start.year ) %>% rename(Year = Yr) #& Yr <=2116
proj.index$low <- NA
proj.index$med <- NA
proj.index$high <- NA
